    ...
    return (
      <>
        <div className="body">
          ...
          <div className="categories">
            {ctgList.data.map((ctg) => (
              <div
                className="category"
                key={ctg._id}
                onClick={() => this.categorySearched(ctg.category)}
              >
                <div className="categoryCard">
                  <div className="ctgLogo">
                    <i
                      className={ctg.icon}
                      style={{ backgroundColor: ctg.bgcolor }}
                    />
                  </div>
          ...
          <div className="productsList">
            {this.state.productsList.slice(0, this.state.showPrd).map((prd) => (
              <div className="prdCard" key={prd.product_id}>
                ...
                <div className="prdDes">
                  ...
                  <br />
                  <span style={{ color: "green" }}>{prd.productPrice}₹</span>
                  <br />
            ...
            <span
              style={{
                ...
                display:
                  this.state.productsList.length === 0 ? "block" : "none",
              }}
            >
              {this.state.mssg}
            </span>
          ...
          <div
            style={{ display: this.state.showBtn ? "flex" : "none" }}
            className="moreBtns"
          >
            <button
              onClick={() => this.setState({ showPrd: this.state.showPrd - 2 })}
              style={{ display: this.state.showPrd <= 2 ? "none" : "block" }}
            >
              show less
            </button>
            <button
              ...
              style={{
                display:
                  this.state.showPrd >= this.state.productsList.length
                    ? "none"
                    : "block",
              }}
            >
          ...
          <div className="textMsgBox">
            <div className="msgBox">
              a small scale apllication to sell products in range of Kotdwara &
              Najibabad.
            </div>
            <div className="msgBox">
              Search, Check, Chat & call in a particular place, purchase your
              Product.
            </div>
            <div className="msgBox">
              No online payment, meet face to face, take phone number in
              chatbox.
            </div>
          </div>
